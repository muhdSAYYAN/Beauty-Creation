...
const filteredproduct = cometics.slice(12,18).filter((product)=>
...
const filteredItems = filteredbycategory();
  ...
  <div className="brandDiv">
    ...
    {filteredItems.length > 0 ? (
          ...
          <Link className='addtocart' to='/detailing' state={{selectedProduct:item}}><span >ADD TO BAG . ₹{item.Pprice} <label className='offer'>₹{item.Rprice}</label></span></Link>
        </div>
